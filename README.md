...
Скопируйте `.env.example` в `.env` и заполните настройки email. Telegram и vmess-прокси необязательны:
...
- `EMAIL_FROM`: обязательный адрес отправителя
- `EMAIL_TO`: адрес получателя заявок

Успех заявки возвращается после принятия письма SMTP-сервером. Если email не настроен или отправка завершилась ошибкой, API возвращает 502. Telegram получает дополнительную копию после отправки письма: его ошибки не влияют на ответ формы. Эта копия отправляется в фоне без гарантии доставки при остановке процесса.
